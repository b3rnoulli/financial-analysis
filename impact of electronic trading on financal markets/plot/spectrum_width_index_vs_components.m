...
if save_files
    fid = fopen(['9-companies-spectrum-width-index-vs-components-mean.csv'], 'w') ;
    fprintf(fid,['window_end_date,','index-width,','components-mean-width\n']);
    
    for j=1:length(date_points)
        fprintf(fid,[datestr(date_points(j),'dd-mm-yyyy'),',',num2str(index_data.alpha_y(j)),',',num2str(mean_width(j)),'\n']);
    end
    fclose(fid);
end
