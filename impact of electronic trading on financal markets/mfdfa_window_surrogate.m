...
for i=1:length(indexes(:,1))
    ...
    while end_index < find_index(data.date,indexes{i,3})
        ...
        fourier_surrogate_file_name = [indexes{i,1},'-fourier-surrogate-',datestr(data.date(start_index)),...
            '-',datestr(data.date(end_index))];
        
        shuffled_surrogate_file_name = [indexes{i,1},'-shuffled-surrogate-',datestr(data.date(start_index)),...
            '-',datestr(data.date(end_index))];
        ...
        fourier_surrogate_mfdfa_file_name =  [indexes{i,1},'-fourier-surrogate-spectrum-',datestr(data.date(start_index)),...
            '-',datestr(data.date(end_index))];
        
        shuffled_surrogate_mfdfa_file_name =  [indexes{i,1},'-shuffled-surrogate-spectrum-',datestr(data.date(start_index)),...
            '-',datestr(data.date(end_index))];
