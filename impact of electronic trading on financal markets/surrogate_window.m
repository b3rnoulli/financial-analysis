...
for i=1:length(indexes(:,1))
    path = ['/Users/b3rnoulli/Development/Matlab workspace/empirical data/',indexes{i,1},'/surrogate/window/'];
    ...
    while end_index < find_index(data.date,indexes{i,3})
        ...
        for j=1:surrogate_count
            fourier_surrogate = surrogate_fourier(data.returns(start_index:end_index));
            standard_surrogate = surrogate_standard(data.returns(start_index:end_index));
